...
zsurf = A(n); n = n + 1;
Nt = A(n);  n = n + 1;
lambda_start = 1250e-7;% start wavelength 
lambda_stop = 1350e-7;% end wavelength 
samplePoints = 1024;
vessel_center_depth = 0.8;% mm, make sure it is the same as GenTissue.m
blood_radius = 0.3;% mm, make sure it is the same as GenTissue.m
n_cor = 1.33; % Refractive index
...
range_z1 = 1;% display range
range_z2 = 300;
range_x1 = 50;% display range
range_x2 = 462;
...
figure;
imagesc(x(range_x1:range_x2),z(range_z1:range_z2),10.*log10(mean1(range_z1:range_z2,range_x1:range_x2)));
colormap gray
title('OCT B-scan (log scale)')
figure;
imagesc(x(range_x1:range_x2),z(range_z1:range_z2),decor_map_filtered(range_z1:range_z2,range_x1:range_x2));
colormap gray
title('OCTA')
%% Q-OCTA

d1 = decor_map_filtered(range_z1:range_z2,range_x1:range_x2);
x1 = x(range_x1:range_x2)*10;
z1 = z(range_z1:range_z2)*10/n_cor;
x0 = 0;
z0 = vessel_center_depth;
distances = zeros(range_z2-range_z1+1,range_x2-range_x1+1);
for i= 1:range_z2-range_z1+1
    for j=1:range_x2-range_x1+1
